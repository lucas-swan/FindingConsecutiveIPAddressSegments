# 主要功能是查找并输出去掉一个不可选 IP 后的最大连续 IP 地址段及相关信息
# 使用python编写程序,实现查找连续的未使用的IP地址段
# 1.输入您使用的IP地址,以空格隔开
# 2.输出去掉一个不可选IP后的最大连续IP地址段
# 3.输出去掉一个不可选IP后的最大连续IP地址段长度
# 4.输出被去掉的IP
def ip_to_int(ip):
...
def find_max_continuous_with_one_removed(ips_list, not_allowed_ips):
    max_length = 0
    max_continuous_ips = []
    removed_ip = ""
    ...
    return max_continuous_ips, max_length, removed_ip
...
max_continuous_ips_with_removed, max_length_with_removed, removed_ip = find_max_continuous_with_one_removed(all_ips, not_allowed_ips)

print(f"去掉一个不可选IP后的最大连续IP地址段: {max_continuous_ips_with_removed}")
print(f"去掉一个不可选IP后的最大连续IP地址段长度: {max_length_with_removed}")
print(f"被去掉的IP: {removed_ip}")
